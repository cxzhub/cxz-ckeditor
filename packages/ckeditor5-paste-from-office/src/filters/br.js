...
export default function transformBlockBrsToParagraphs (
  ...
  for (const value of writer.createRangeIn(documentFragment)) {
    ...
    if (element.is('element', 'br')) {
      ...
      const previousSiblingIsBlock = isBlockViewElement(
        previousSibling,
        blockElements
      )
...
function findSibling (
  ...
  direction,
  writer,
  { blockElements, inlineObjectElements }
) {
  let position = writer.createPositionAt(
    viewElement,
  ...
  position = position.getLastMatchingPosition(
    ({ item }) =>
      item.is('element') &&
