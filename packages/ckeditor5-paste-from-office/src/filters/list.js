...
export function transformListItemLikeElementsIntoLists (
  ...
  itemLikeElements.forEach((itemLikeElement, i) => {
    const isDifferentList = isNewListNeeded(
      itemLikeElements[i - 1],
      itemLikeElement
    )
    const previousItemLikeElement = isDifferentList
      ? null
      : itemLikeElements[i - 1]
    const indentationDifference = getIndentationDifference(
      previousItemLikeElement,
      itemLikeElement
    )
    ...
    const listItem = transformElementIntoListItem(
      itemLikeElement.element,
      writer
    )
...
function findAllItemLikeElements (documentFragment, writer) {
  ...
  for (const value of range) {
    if (
      ...
      itemLikeElementsMatcher.match(value.item)
    ) {
...
function detectListStyle (listLikeItem, stylesString) {
  const listStyleRegexp = new RegExp(
    `@list l${listLikeItem.id}:level${listLikeItem.indent}\\s*({[^}]*)`,
    'gi'
  )
  const listStyleTypeRegex = /mso-level-number-format:([^;]{0,100});/gi
  const listStartIndexRegex =
    /mso-level-start-at:\s{0,100}([0-9]{0,10})\s{0,100};/gi
  ...
  if (listStyleMatch && listStyleMatch[1]) {
    ...
    if (listStyleTypeMatch && listStyleTypeMatch[1]) {
      ...
      type =
        listStyleType !== 'bullet' && listStyleType !== 'image' ? 'ol' : 'ul'
    }
